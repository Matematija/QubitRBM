...
def adam_hadamard(rbm, lr, n, n_steps, betas=(0.9, 0.999), eps=1e-8, psi_mcmc_params=(500, 100, 1), phi_mcmc_params=(2000, 500, 1), verbose=False):
    ...
    phi_samples = rbm.get_samples(**phi_mcmc_args, state='h', n=n)
    phiphi = rbm.eval_H(phi_samples, n=n)
    ...
    v = np.zeros_like(params, dtype=np.float)

    for t in range(n_steps):
        
        psi_samples = logpsi.get_samples(**psi_mcmc_args)

        phipsi = rbm.eval_H(psi_samples, n=n)
...
def hadamard_optimization(rbm, n, tol=1e-6, lookback=50, psi_mcmc_params=(500, 10, 100, 1), phi_mcmc_params=(2000, 10, 500, 1),
    ...
    # while (np.abs(F_mean_new - F_mean_old) > tol or t < 2*lookback + 1) and F < 0.99:
        ...
        if time() - clock > 20 and verbose:
            print('Iteration {:4d} | Fidelity = {:05.4f} | lr = {:04.3f} | diff_mean_F = {:08.7f}'.format(t, F, lr_, F_mean_new - F_mean_old))
            clock = time()

    return params, history

def parallel_hadamard_optimization(rbm, comm, n, tol=1e-6, lookback=50, psi_mcmc_params=(500, 100, 1), phi_mcmc_params=(2000, 500, 1),
    ...
    return logpsi.get_flat_params(), history
