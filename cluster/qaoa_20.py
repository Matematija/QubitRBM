...
beta_1 = np.pi/4 # fixing \beta_1

gammas = np.linspace(0, np.pi/4, size)
gamma_1 = gammas[r]
...
for n in range(nq):
    ...
    params, Fs = rx_optimization(logpsi, n, beta_1, tol=tol, lr=lr, lookback=10, resample_phi=1, sigma=0.0,
                                   psi_mcmc_params=(2000,3,200,30), phi_mcmc_params=(2000,3,200,30),
...
save_folder = os.path.join(os.getcwd(), 'output_data')
